...
// 重置按钮位置到面板内部
function resetButtonPositions(side) {
    const panel = side === 'left' ? elements.leftPanel : elements.rightPanel;
    const btn = side === 'left' ? elements.foldLeftBtn : elements.foldRightBtn;
    ...
    // 重置按钮样式
    btn.style.right = side === 'left' ? '-20px' : 'auto';
    btn.style.left = side === 'left' ? 'auto' : '-20px';
...
function startTypingAnimation(text) {
    ...
    state.typingInterval = setInterval(() => {
        if (index < originalText.length) {
            elements.dialogInput.value += originalText[index];
            index++;
            
            // 自动滚动到底部
            elements.dialogInput.scrollTop = elements.dialogInput.scrollHeight;
        } else {
            stopTypingAnimation();
        }
    }, speed);
...
function nextDialog() {
    ...
    // 更新对话框显示（直接显示完整文本，不播放动画）
    updateDialogDisplay();
    
    // 如果开启了打字机动画且下一句存在，自动播放打字效果
    if (state.animationEnabled) {
        const nextIndex = currentScene.currentDialogIndex;
        if (nextIndex < currentScene.dialogs.length) {
            const dialog = currentScene.dialogs[nextIndex];
...
function togglePanel(panel) {
    if (panel === 'left') {
        ...
        // 更新按钮图标
        if (state.leftPanelFolded) {
            elements.foldLeftBtn.textContent = '▶';  // 折叠状态显示展开图标
        } else {
            elements.foldLeftBtn.textContent = '◀';  // 展开状态显示折叠图标
        }
    } else if (panel === 'right') {
        // 切换右边面板的折叠状态
        ...
        // 更新按钮图标
        if (state.rightPanelFolded) {
            elements.foldRightBtn.textContent = '◀';  // 折叠状态显示展开图标
        } else {
            elements.foldRightBtn.textContent = '▶';  // 展开状态显示折叠图标
        }
    }
}
...
async function exportAsGif() {
    ...
    try {
        ...
        const gif = new GIF({
            ...
            const frameDelay = state.animationSpeed;
...
// 导出所有场景为GIF
async function exportAllAsGifs() {
    if (state.scenes.length === 0) {
        alert('没有可导出的场景');
        return;
    }
    ...
    for (let i = 0; i < state.scenes.length; i++) {
        ...
        if (!currentScene.dialogs || currentScene.dialogs.length === 0) {
            console.warn(`场景 ${i + 1} 没有对话内容，跳过`);
            continue;
        }
        
        const dialog = currentScene.dialogs[0];
        const text = dialog.text || '';
        ...
        try {
            ...
            // 保存原始文本
            const originalText = elements.dialogInput.value;
            const originalName = elements.characterName.value;
            elements.characterName.value = dialog.character || '???';
            ...
            // 恢复原始文本
            elements.dialogInput.value = originalText;
            elements.characterName.value = originalName;
            
            // 导出GIF
            gif.on('finished', (blob) => {
                const url = URL.createObjectURL(blob);
                const link = document.createElement('a');
                link.download = `scene_${i + 1}.gif`;
                link.href = url;
                link.click();
            });
    ...
    alert(`已导出 ${state.scenes.length} 个GIF`);
}
